...
static saan_kanji_status run_after_layout(const jdict_t *d, const char *text, size_t nbytes,
                                          ...
                                          int32_t *ids, int32_t ids_cap,
                                          int32_t *n_ids, int *n_tokens) {
    /* text2mecab needs NUL termination; the parser API accepts a byte span.
     * Keep both buffers on this task's stack, outside the shared arena. */
    char input[KJ_KEY_MAX];
    char normalized[KJ_KEY_MAX];
    if (nbytes >= sizeof(input)) return SAAN_KANJI_ERR_TOO_LONG;
    memcpy(input, text, nbytes);
    input[nbytes] = '\0';
    const text2mecab_result_t normalized_status = text2mecab(normalized, sizeof(normalized), input);
    if (normalized_status == TEXT2MECAB_RESULT_RANGE_ERROR) return SAAN_KANJI_ERR_TOO_LONG;
    if (normalized_status != TEXT2MECAB_RESULT_SUCCESS) return SAAN_KANJI_ERR_KEY;
    text = normalized;
    nbytes = strlen(normalized);
    ...
    int node_count = 0;
    for (NJDNode *p = njd.head; p; p = p->next) ++node_count;
    if (node_count > KJ_MAX_TOK) {
        JPCommon_clear(&jp);
        NJD_clear(&njd);
        return SAAN_KANJI_ERR_TOO_LONG;
    }
    int nk = 0;
